...
export default function ReviewLesson({ pool, onClose, onFinished, playWord, speaking = '' }: Props) {
  ...
  return <div ref={dialog} className="lesson-overlay" role="dialog" aria-modal="true" aria-labelledby="lesson-heading" onKeyDown={event => {
    if (event.defaultPrevented || event.key !== 'Enter' || event.nativeEvent.isComposing || event.keyCode === 229 || event.target instanceof HTMLButtonElement) return;
