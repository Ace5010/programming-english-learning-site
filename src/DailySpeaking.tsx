...
export default function DailySpeaking({ exercise, draft, disabled, speaking, play, stopAudio, onChange, onBusyChange }: Props) {
  ...
  function listen(phrase: DailyPhrase, slow = false) { speech.abort(); play(phrase, slow); }
  ...
  const notice = <div className="speech-service-note">
    <p>{speech.supported ? speech.local ? '使用设备上的英语识别。识别文字随进度保存，本站不保存录音。' : `使用${isAndroidApp() ? '手机系统' : '浏览器'}语音服务，声音可能发送至该服务。识别文字随进度保存，不保存录音。` : window.isSecureContext ? '当前浏览器不支持语音识别。可换用支持此功能的 Chrome，或选择“自己表达”继续。' : '麦克风需要安全连接。请使用 HTTPS 或本机 localhost 地址打开，或选择“自己表达”继续。'}</p>
  </div>;
    ...
    {mode === 'read' ? <>
      ...
      <div className="speech-targets">{exercise.readAloud?.map(phrase => {
        ...
        return <article className={`speech-target${comparison?.allMatched ? ' matched' : ''}`} key={phrase.id} data-phrase-id={phrase.id}>
          ...
          <div className="speech-actions"><button type="button" className={`daily-audio${playing ? ' playing' : ''}`} aria-label={`听示范 ${phrase.en}`} aria-pressed={playing} disabled={speech.busy} onClick={() => listen(phrase, false)}><Icon name="sound" />听示范</button><button type="button" className={`daily-inline-slow${speaking === `daily-${phrase.id}-slow` ? ' playing' : ''}`} aria-pressed={speaking === `daily-${phrase.id}-slow`} aria-label={`慢速朗读 ${phrase.en}`} disabled={speech.busy} onClick={() => listen(phrase, true)}>慢速</button>{mic(phrase.id, phrase.en)}</div>
