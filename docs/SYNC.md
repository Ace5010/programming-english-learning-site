...
2026-09-23。实际学习站点为 https://programming-english-learning-site.pages.dev/ 。Android 1.1.2 使用同一同步接口，仍保留完整离线资源与原有学习记录。

## 使用

1. 在原来有学习进度的网页刷新一次，点击顶部“同步”→“开启同步”。等待“进度已同步”，再复制同步码。
2. 安装 Android 1.1.2，在“同步”中粘贴这个码，点击“连接已有进度”。无需注册账号。
...
1.1.2 的点读修复沿用这套同步协议、存储键及原签名；已配对设备覆盖升级后无需重新配对。点读期间暂缓应用远端进度，避免重建课程打断录音，结束后继续既有自动同步流程。此次通过隔离 SQLite 的 8 组浏览器同步检查，没有重跑真实 D1 同步；既有端到端证据如下。
...
- 206 项 Node 回归通过，含 20 项同步与 SQL 测试；8 组隔离浏览器同步场景通过，覆盖同源多标签页。
- Cloudflare 正式部署 `01f15634-b279-4cbf-8a15-1c27dca39f9f` 对应提交 `a9db792b`，2026-09-23 10:21（北京时间）显示成功；网页构建与本地 APK 使用相同资源版本。
- 部署后真实 D1 双端浏览器链路 7 组通过：旧记录首次迁移、网页收藏到手机、手机离线补传到网页、刷新后的配对与持久化、冲突拦截／备份／选择、断开后保留进度，以及四风格小屏布局。报告不包含同步码，使用隔离测试数据。
- Android 1.1.0（versionCode 2）Release / Debug 构建及 Lint 通过；v2 签名验证通过，签名证书与 1.0.0 相同。14,665 个包内网页文件与 dist 哈希一致，14,648 个 MP3 完整。
- APK SHA-256：`b777465f80a63f00770eab4e7fa5147f96f7c687ca4df7f6fc1cddbd4535d2b6`。真机安装、真实系统识别和文件选择器尚未验收；Chrome 中的 APK origin 仿真不能替代真机结果。
